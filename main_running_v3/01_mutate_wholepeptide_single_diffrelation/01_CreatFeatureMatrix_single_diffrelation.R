...
#### 02 基于相关性的特征筛选 ####
# 2.1 未经过特征筛选的相关性展示 ###
output_path <- "./result_v3/01_mutate_wholepeptide_single_diffrelation_result/01_相关性计算/"
...
mutate.property_Cor_filtered <- caret::findCorrelation(mutate.property_Cor,cutoff = 0.75,verbose = F,names = T)
mutate.property_filtered <- as.data.table(mutate.property)[,-mutate.property_Cor_filtered,with=F]
CorPlot(mutate.property_filtered,
        outputname =paste0(output_path,"AllCorrelation.filtered75_hclust_circle.pdf"),
        width = 24,
        height = 24,
        order = "hclust",
        corvismethod = "circle")

# correlation by different cor cutoff and then save output
#need
#output_path
#mutate_property
#final_result_v2
Cor_filtered_fun <- function(cutoffset = 0.75){
  mutate.property_Cor_filtered <- caret::findCorrelation(mutate.property_Cor,cutoff = cutoffset,verbose = F,names = T)
  mutateML <- mutate.property %>%
    as.data.table() %>%
    .[,-mutate.property_Cor_filtered,with=F] %>%
